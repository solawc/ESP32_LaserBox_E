...
/***********************************************************************************
 * 这里使用的是RTOS中的互斥锁作，由于ESP32不像STM32这些ARM系列MCU，所以它不能通过中断屏蔽
 * 的方式来进入临界段，临界本身的意义在于类似裸机一样的连续操作，他需要时间短、速度快的代码。
 * 因此使用互斥的方式保护资源
 * ********************************************************************************/
// inner lock
...
static int i2s_out_stop() {
    ...
    // Stop FIFO DMA
    // I2S0.out_link.stop = 1;  wait fix

    // Disconnect DMA from FIFO
    // I2S0.fifo_conf.dscr_en = 0;  //Unset this bit to disable I2S DMA mode. (R/W)   wait fix
...
static int i2s_out_start() {
    ...
    // reset DMA
    // I2S0.lc_conf.in_rst  = 1;    wait fix
    // I2S0.lc_conf.in_rst  = 0;
    // I2S0.lc_conf.out_rst = 1;
    // I2S0.lc_conf.out_rst = 0;

    // I2S0.out_link.addr = (uint32_t)o_dma.desc[0]; wait fix
    ...
    // start DMA link
    if (i2s_out_pulser_status == PASSTHROUGH) {
        // I2S0.conf_chan.tx_chan_mod = 3;  // 3:right+constant 4:left+constant (when tx_msb_right = 1) wait fix
        I2S0.conf_single_data      = port_data;
    } else {
        // I2S0.conf_chan.tx_chan_mod = 4;  // 3:right+constant 4:left+constant (when tx_msb_right = 1) wait fix
    ...
    // Connect DMA to FIFO
    // I2S0.fifo_conf.dscr_en = 1;  // Set this bit to enable I2S DMA mode. (R/W) wait fix

    I2S0.int_clr.val    = 0xFFFFFFFF;
    // I2S0.out_link.start = 1; // wait fix

    // I2S0.conf.tx_start = 1; // wait fix
        ...
        while (o_dma.rw_pos < (DMA_SAMPLE_COUNT - SAMPLE_SAFE_COUNT)) {
            ...
            if (i2s_out_remain_time_until_next_pulse < I2S_OUT_USEC_PER_PULSE) {
                ...
                if (i2s_out_pulser_status == STEPPING) {
                    ...
                    // Stepper::pulse_func(); // 这里执行脉冲输出回调，pulse_func是执行在中断里面的函数
...
int i2s_out_init() {
    ...
    uint8_t wsPin   = GPIO_NUM_17;  // i2so->_ws;
    uint8_t bckPin  = GPIO_NUM_16;  // i2so->_bck;
    uint8_t dataPin = GPIO_NUM_21;  // i2so->_data;

    i2s_out_init_t default_param;
    default_param.ws_pin       = wsPin;
    default_param.bck_pin      = bckPin;
    default_param.data_pin     = dataPin;
